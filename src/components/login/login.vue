<template lang="html">
  ...
  <div slot="title" class="loginHeader">
    <div :class="['tab',tabIndexClass]">
      ...
      <a href="javascript:;" @click="tab(2)" :class="[mobileOrEmail==2?'active':'']">
  ...
  <el-form-item label="" prop="mobile" class="mobile" v-if="mobileOrEmail==1">
    <div class="mobileAreaSelect">
      ...
      <el-select class="p-a-select" v-model="loginAndRegister.area" @click="loginAndRegister.mobile=''">
        <el-option v-for="item in phoneArea" :value="item" :label="item"></el-option>
      </el-select>
    </div>
  ...
  <p class="register" v-show="optionsType==1">{{$t("login.30")}}？<span @click="optionsType = 2;tab(mobileOrEmail)">立即{{$t('login.7')}}</span></p>
    ...
    methods: {
        ...
        loginSubbmit() {
            ...
            this.http(
                ...
                data => {
                    store.commit("loginStatusCtr", true);
                    // console.log(data);
